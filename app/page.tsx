...
export default function Home() {
  ...
  if (running && current) return <main className="player" style={{ "--accent": current.exercise.color } as React.CSSProperties}>
    <audio ref={audioRef} src={playbackUrl ?? undefined} loop /><audio ref={gongRef} src="/sounds/boxing-round-double.mp3" preload="auto" /><audio ref={announcementRef} preload="auto" /><div className="playerTop"><div className="roundStatus"><span>RUNDA {current.roundIndex + 1} AV {rounds.length}</span><small>{current.roundName}</small></div><button className="endWorkout" onClick={() => { setRunning(false); setPreStarting(false); setPreStartCount(0); setRoundPausing(false); if ("speechSynthesis" in window) window.speechSynthesis.cancel(); audioRef.current?.pause(); }}><b aria-hidden="true">■</b> Avsluta</button></div>
