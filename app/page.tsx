...
export default function Home() {
  ...
  const speakCloud = (text: string, onEnd?: () => void) => { const trimmed = text.trim(); if (!trimmed) { onEnd?.(); return; } const ctx = getSoundContext(); const playBuffer = (buffer: AudioBuffer) => { const source = ctx.createBufferSource(); source.buffer = buffer; source.connect(ctx.destination); source.onended = () => onEnd?.(); source.start(0); }; const cachedBuffer = ttsBufferCacheRef.current.get(trimmed); if (cachedBuffer) { playBuffer(cachedBuffer); return; } const decode = (url: string) => fetch(url).then(res => res.arrayBuffer()).then(buf => ctx.decodeAudioData(buf)).then(buffer => { ttsBufferCacheRef.current.set(trimmed, buffer); playBuffer(buffer); }).catch(() => onEnd?.()); const cachedUrl = ttsCacheRef.current.get(trimmed); if (cachedUrl) { decode(cachedUrl); return; } fetch(`/api/tts?code=${syncCode}&text=${encodeURIComponent(trimmed)}`).then(res => res.ok ? res.json() : Promise.reject()).then(data => { ttsCacheRef.current.set(trimmed, data.url); decode(data.url); }).catch(() => { onEnd?.(); }); }; const playAnnouncement = (exercise?: Exercise, preview = false, first = false, onComplete?: () => void) => {
