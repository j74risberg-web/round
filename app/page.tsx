...
export default function Home() {
  ...
  const [musicPickerExerciseId, setMusicPickerExerciseId] = useState<string | null>(null);
  ...
  const playBufferLegacyVoice = (buffer: AudioBuffer, token: number, outputBoost = 1) => new Promise<void>((resolve) => {
    ...
    const source = ctx.createBufferSource();
    const gain = ctx.createGain();
    // Återställ den äldre ljudvägen som uttalade Armhävningar korrekt:
    // ingen kompressor och den ursprungliga TTS-förstärkningen.
  ...
  useEffect(() => { const audio = audioRef.current; if (!audio) return; if (running && !paused && !musicPaused && !preStarting && !roundPausing && playbackUrl) audio.play().catch(() => {}); else audio.pause(); }, [running, paused, musicPaused, preStarting, roundPausing, playbackUrl]);
  useEffect(() => { setMusicPaused(false); }, [playbackUrl]);
  ...
  >
    ...
    </> : <><div className="sectionTitle"><div><h2>Musikbibliotek</h2><p>{musicLibrary.length} låtar · ladda upp en gång och återanvänd överallt.</p></div></div><label className={`musicLibraryUpload${uploadingLibraryMusic ? " uploading" : ""}`} aria-busy={uploadingLibraryMusic}>{uploadingLibraryMusic ? <><span className="uploadSpinner" aria-hidden="true" /> Laddar upp…</> : <>＋ Lägg till låt</>}<input type="file" accept="audio/*,.mp3,.m4a,.wav,.aac" disabled={uploadingLibraryMusic} onChange={e => { const input = e.currentTarget; void addMusicToLibrary(input.files?.[0]).finally(() => { input.value = ""; }); }} /></label><div className="trackLibrary">{[...musicLibrary].sort((a, b) => a.name.localeCompare(b.name, "sv", { sensitivity: "base" })).map(track => <article key={track.id} className={previewTrackId === track.id ? "isPlaying" : ""}><button className="trackPlay" onClick={() => toggleTrackPreview(track)} aria-label={previewTrackId === track.id ? `Stoppa ${track.name}` : `Spela ${track.name}`} aria-pressed={previewTrackId === track.id}>{previewTrackId === track.id ? <span className="playingBars" aria-hidden="true"><i /><i /><i /></span> : "▶"}</button><div><strong>{track.name}</strong><span>Kan väljas på valfri övning</span></div><span className="trackActions">{track.bundled && <span className="bundledTrack">I appen</span>}<button className="trackDelete" onClick={() => removeMusicFromLibrary(track)} aria-label={`Ta bort ${track.name}`}>Ta bort</button></span></article>)}</div></>}{musicPickerExerciseId && (() => { const pickerExercise = exercises.find(exercise => exercise.id === musicPickerExerciseId); if (!pickerExercise) return null; const pickerTracks = [...musicLibrary].sort((a, b) => a.name.localeCompare(b.name, "sv", { sensitivity: "base" })).filter(track => track.name.toLocaleLowerCase("sv").includes(musicPickerSearch.trim().toLocaleLowerCase("sv"))); const closePicker = () => { const current = previewAudioRef.current; if (current) { current.pause(); current.currentTime = 0; previewAudioRef.current = null; } setPreviewTrackId(null); setMusicPickerExerciseId(null); setMusicPickerSearch(""); }; const chooseTrack = (track?: MusicTrack) => { updateExercise(pickerExercise.id, track ? { music: track.url, musicName: track.name } : { music: "none", musicName: undefined }); closePicker(); }; return <div className="musicPickerBackdrop" role="presentation" onClick={closePicker}><section className="musicPickerSheet" role="dialog" aria-modal="true" aria-labelledby="music-picker-title" onClick={event => event.stopPropagation()}><div className="musicPickerHeader"><div><h3 id="music-picker-title">Välj musik</h3><p>Till: {pickerExercise.name}</p></div><button type="button" className="musicPickerClose" onClick={closePicker} aria-label="Stäng musikväljaren">×</button></div><div className="musicPickerSearchWrap"><span aria-hidden="true">⌕</span><input value={musicPickerSearch} onChange={event => setMusicPickerSearch(event.target.value)} placeholder="Sök låt…" /></div><div className="musicPickerList"><button type="button" className={`musicPickerNone${!pickerExercise.music || pickerExercise.music === "none" ? " selected" : ""}`} onClick={() => chooseTrack()}><span className="musicPickerNoneIcon">∅</span><span><strong>Ingen musik</strong><small>Spela övningen utan musik</small></span>{(!pickerExercise.music || pickerExercise.music === "none") && <b aria-label="Vald">✓</b>}</button>{pickerTracks.length ? pickerTracks.map(track => { const selected = pickerExercise.music === track.url; const playing = previewTrackId === track.id; return <article key={track.id} className={`musicPickerTrack${selected ? " selected" : ""}${playing ? " playing" : ""}`}><button type="button" className="musicPickerPlay" onClick={() => toggleTrackPreview(track)} aria-label={playing ? `Stoppa ${track.name}` : `Provspela ${track.name}`}>{playing ? <span className="playingBars" aria-hidden="true"><i /><i /><i /></span> : "▶"}</button><button type="button" className="musicPickerChoose" onClick={() => chooseTrack(track)}><span><strong>{track.name}</strong><small>{track.bundled ? "I appen" : "Uppladdad låt"}</small></span>{selected && <b aria-label="Vald">✓</b>}</button></article>; }) : <div className="musicPickerEmpty">Ingen låt matchar sökningen.</div>}</div><button type="button" className="musicPickerCancel" onClick={closePicker}>Avbryt</button></section></div>; })()}<nav className="bottomNav"><button className={tab === "pass" && !editingPass ? "active" : ""} onClick={() => { setTab("pass"); setEditingPass(false); }}><i>◉</i><span>Pass</span></button><button className={tab === "ovningar" ? "active" : ""} onClick={() => setTab("ovningar")}><i>▦</i><span>Övningar</span></button><button className={tab === "musik" ? "active" : ""} onClick={() => setTab("musik")}><i>♫</i><span>Musik</span></button><button className={tab === "pass" && editingPass ? "active" : ""} onClick={() => { setTab("pass"); setEditingPass(true); }}><i>⚙</i><span>Redigera</span></button></nav>{tab === "pass" && editingPass && <footer>Synkas mellan enheter med samma kod. Sparas även lokalt som säkerhetskopia.</footer>}
  </main>;
}
